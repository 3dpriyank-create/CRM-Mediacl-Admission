...
export class BaseRepository<T extends {id?:string}>{constructor(private collection:string){}
 ...
 async create(data:Omit<T,'id'>){const doc=this.ref().doc();const payload={...data,id:doc.id,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()};await doc.set(payload);return payload as T;}
